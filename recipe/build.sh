...
for _TYPE in cli gui; do
  ...
  if [[ "${c_compiler}" == "vs" ]]; then
    cl.exe -D NDEBUG -D "WIN32_LEAN_AND_MEAN" ${CPPFLAGS} -ZI -Gy -MT launcher.c -Os -link -MACHINE:${_CL_MACHINE} ${LDFLAGS} resources-${_ARCH}.res version.lib advapi32.lib shell32.lib -out:${_TYPE}-${_ARCH}.exe
  else
