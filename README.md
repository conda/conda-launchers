...
`conda-launchers` follows a `YY.MM.MICRO.BUILDNUM` CalVer scheme. The `YY.MM.MICRO` was initially seeded from `conda-build`, where the patches and scripts used to live; subsequent releases will stick to the release month. The `BUILDNUM` component is taken from the build number in `recipe/recipe.yaml`.
